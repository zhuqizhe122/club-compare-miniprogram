...
## Phase 9：条件筛选分支整合（US4）

- [x] T052 [US4] 在首页增加“按条件筛选”意图入口，复用 `pages/library` 且不增加第九页
- [x] T053 [US4] 补齐分类、时段、投入、互动、活动节奏、活动形式、门槛和证据状态的交集筛选
- [x] T054 [US4] 验证筛选结果可进入 `pages/club`，返回后条件和共享候选状态不丢失
- [x] T055 运行项目、数据、引擎、页面和规则全部 Node 回归
- [ ] T056 在微信开发者工具中人工验收三个首页入口、组合筛选、详情返回和小屏布局
...
任务 ID 为 T001–T056；用户故事任务均带 `[USn]`，可并行且不写同一文件的任务标记 `[P]`，每项均给出目标路径。
